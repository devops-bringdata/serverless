...
describe('Enrichment', () => {
  test('should return 400 if no campaignId is provided', async () => {
    const { sut } = makeSut()
    const response = await sut.start(null, 'any_group_id', 'bringdata-jest')
  ...
  test('should return 400 if no groupId is provided', async () => {
    const { sut } = makeSut()
    const response = await sut.start('any_campaign_id', null, 'bringdata-jest')
  ...
  test('should call findCampaign with correct campaignId', async () => {
    ...
    await sut.start('any_campaign_id', 'any_group_id', 'bringdata-jest')
    expect(dbFindCampaignSpy.campaignId).toBe('any_campaign_id')
    expect(dbFindCampaignSpy.schemaName).toBe('bringdata-jest')
  })
  test('should call findRows with correct groupId', async () => {
    ...
    expect(dbFindUploadedDataByGroupIdSpy.schemaName).toBe('bringdata-jest')
  })
})
